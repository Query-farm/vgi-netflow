# CI: the vgi-netflow worker integration suite
...
(`test/sql/*.test`) against the vgi-netflow VGI worker through the **real DuckDB
...
| `TRANSPORT`  | `VGI_NETFLOW_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/netflow-worker`          | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `netflow-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/netflow.<pid>.sock`           | `netflow-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
1. **Build the worker** — `cargo build --release --bin netflow-worker`. The
   compiled `target/release/netflow-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_NETFLOW_WORKER}`).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-netflow `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_NETFLOW_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_NETFLOW_WORKER` accordingly, stages the
...
WORKER_BIN="$PWD/target/release/netflow-worker" \
TRANSPORT=subprocess \
  ci/run-integration.sh

# HTTP leg: launches `netflow-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/netflow-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `netflow-worker --unix /tmp/netflow.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/netflow-worker" \
