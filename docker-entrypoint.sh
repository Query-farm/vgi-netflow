...
#
# Dispatch the single vgi-netflow image into one of its transports:
...
#
# The worker holds no on-disk state (its template cache is externalized as VGI
# scan state over the protocol), so there is no /data to create and no state env
# to wire — each mode just exec's the binary.
set -e
...
case "${1:-http}" in
  http)
    ...
    exec netflow-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec netflow-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec netflow-worker "$@"
