#!/usr/bin/env bash
# Build the browser (`worker:`) build of the netflow VGI worker:
# netflow-wasm compiled to wasm32-unknown-emscripten, linked by emcc into a
...
LIB="target/$TARGET/release/libnetflow_wasm.a"
...
# of bounds" with no console output. INITIAL_MEMORY must cover main stack + every
# pthread stack up front, or startup stalls trying to grow shared memory while the
# pool is spawning.
echo "==> emcc link"
